...
def add_master_table(username, master_pw, fernet_thing):
    pw_encrypted = crypto_stuff.encrypt_text(master_pw, fernet_thing)
    un_encrypted = crypto_stuff.encrypt_text(username, fernet_thing)
    db.add_to_master_table(un_encrypted, pw_encrypted)

# this does not check if we add duplicates
def add_info(info, fernet_thing):
    info_encrypted = crypto_stuff.encrypt_text_list(info, fernet_thing)
    db.add_to_info_table(info_encrypted)


# get from database and decrypt
def get_master_table(fernet_thing):
    master_e = db.get_master_table()
    master = crypto_stuff.decrypt_text_list(master_e, fernet_thing)
    return master

def get_info_decrypted_w_rowid(fernet_thing):
    results_e = db.get_info_w_rowid()
    results = []
    for result in results_e:
        result_d = crypto_stuff.decrypt_text_list(result[1:], fernet_thing)
...
def get_info_for_exact_app(app_name, fernet_thing):
    results_e = db.get_info()
    results = []
    for result in results_e:
        result_d = crypto_stuff.decrypt_text_list(result, fernet_thing)
...
def get_info_for_app(app_name, fernet_thing):
    results_e = db.get_info()
    results = []
    for result in results_e:
        result_d = crypto_stuff.decrypt_text_list(result, fernet_thing)
...
def get_info_for_app_w_rowid(app_name, fernet_thing):
    results_e = db.get_info_w_rowid()
    results = []
    for result in results_e:
...
def get_app_list(fernet_thing):
    app_list = []
    data = get_info_decrypted_w_rowid(fernet_thing)
    for row in data:
        is_new = True
        # check for repeats
        for app in app_list:
...
def change_pw_in_row(rowid, new_pw, fernet_thing):
    new_pw_e = crypto_stuff.encrypt_text(new_pw, fernet_thing)
    db.modify_password_by_rowid(rowid, new_pw_e)

# check if app name and username are in database
# info = (username, email, app, url)
def is_info_in_db(info, fernet_thing):
    possible_rows = get_info_for_exact_app(info[2], fernet_thing)
    if possible_rows == []:
        return False
