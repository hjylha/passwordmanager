...
def test_get_info_from_user(capsys, monkeypatch, app, url, username, email):
    ...
    assert pm_ui.get_info_from_user() == (username, email, app, url)
...
def test_get_changed_info(monkeypatch, info):
    ...
    monkeypatch.setattr(pm_ui, 'yes_or_no_question', answer_to_yn)
    ...
    assert pm_ui.get_changed_info() == new_info
...
def test_print_info(capsys, title, info):
    pm_ui.print_info(title, info)

    email = info[1] if pm_ui.is_valid_email(info[1]) else '-'
...
def test_how_to_generate_pw(monkeypatch, num1, num2):
    count = [0]
    ...
    if num1 in [1, 2, 3]:
        assert pm_ui.how_to_generate_pw() == num1
    else:
        assert pm_ui.how_to_generate_pw() == 0


@pytest.mark.parametrize(
    'num', ['nonsense', 0, 1, 2]
)
def test_generate_pw_generation(num):
    password = pm_ui.generate_pw(num)
    if num == 0:
        assert pm_ui.generate_pw(num) == ''
    if num in [1, 2]:
...
def test_generate_pw_user_pw(monkeypatch, pw1, pw2, continue_no_typing, continue_not_same):
    pw_count = [-1, -1]
    def write_pw(prompt=None):
    ...
    monkeypatch.setattr(pm_ui, 'yes_or_no_question', answer_yn)

    if continue_not_same[-1] == 'n' or continue_no_typing[-1] == 'n':
        assert pm_ui.generate_pw(3) == ''
    else:
        assert pm_ui.generate_pw(3) == pw2[-1]


@pytest.mark.parametrize(
    'num', [0, 1, 2]
)
def test_reveal_password(capsys, monkeypatch, num):
...
def test_obtain_password_no_reveal(capsys, info):
    pm_ui.obtain_password(info, False)
...
def test_obtain_password_w_reveal(capsys, monkeypatch, info, choice):
    def choose(text=None):
        return str(choice)

    monkeypatch.setattr('builtins.input', choose)

    pm_ui.obtain_password(info, True)
    ...
    if choice == 1:
        ...
        assert pm_ui.pyperclip.paste() == info[3]
    elif choice == 2:
